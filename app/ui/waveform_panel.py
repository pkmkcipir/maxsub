...
from ..utils.constants import COLOR_BG_DARK_SURFACE, COLOR_SECONDARY, COLOR_DANGER
...
class WaveformPanel(ctk.CTkFrame):
    def __init__(self, master, on_seek=None, on_region_change=None, **kwargs):
        ...
        self.canvas = tk.Canvas(self, bg=COLOR_BG_DARK_SURFACE, highlightthickness=0)
    ...
    def _draw_regions(self, width, height):
        for line in self.lines:
            ...
            fill = COLOR_SECONDARY if is_selected else "#666666"
            ...
            if is_selected:
                self.canvas.create_line(x1, 0, x1, height, fill=COLOR_SECONDARY, width=2, tags="handle-start")
                self.canvas.create_line(x2, 0, x2, height, fill=COLOR_DANGER, width=2, tags="handle-end")
