...
class MainWindow(ctk.CTk):
    ...
    def _translate(self):
        ...
        self._current_translator = Translator(
            source="auto", target=self.config_manager.get("target_language"),
        ...
        def task():
            try:
                ...
                self.after(0, self._on_lines_updated)
                n_fail = self._current_translator.failed_count
                if n_fail:
                    msg = (f"Terjemahan selesai, tapi {n_fail} baris gagal diterjemahkan "
                           f"(teks asli dipertahankan pada baris itu). Coba klik Terjemahkan "
                           f"lagi nanti, atau edit baris itu manual.")
                else:
                    msg = "Terjemahan selesai"
                self.after(0, self._set_busy, False, msg)
    ...
    def _auto_process(self):
        ...
        def task():
            try:
                ...
                n_fail = self._current_translator.failed_count
                warn = f" - PERINGATAN: {n_fail} baris gagal diterjemahkan (teks asli dipertahankan)" if n_fail else ""
                self.after(0, self._set_busy, False,
                           f"Selesai! {len(lines)} baris diterjemahkan ({detected} -> {target}){warn}")
