...
Ini artinya GPU NVIDIA Anda terdeteksi ada, tapi library CUDA Toolkit
lengkap (cuBLAS/cuDNN) belum terpasang di sistem — umum terjadi kalau
cuma driver GPU biasa yang ter-install, bukan CUDA Toolkit penuh. Sejak
versi ini, MaxSubtitle otomatis mendeteksi kegagalan tersebut dan
langsung mengulang prosesnya memakai CPU tanpa perlu tindakan apa pun
dari Anda — cukup tunggu, transkripsi akan tetap selesai (hanya sedikit
lebih lambat karena berjalan di CPU). Kalau ingin GPU benar-benar
dipakai, install CUDA Toolkit dari https://developer.nvidia.com/cuda-downloads
sesuai versi driver Anda.
