...
9. [Terjemahan Tanpa Limit (LibreTranslate Lokal)](#terjemahan-tanpa-limit-libretranslate-lokal)
...
## Terjemahan Tanpa Limit (LibreTranslate Lokal)

Google Translate gratis & MyMemory (mesin cadangan bawaan) sama-sama punya
batas — Google tidak resmi/kadang diblokir, MyMemory dibatasi 5.000
karakter/hari tanpa email (lihat Troubleshooting). Kalau butuh volume besar
tanpa batas sama sekali dan tanpa biaya, jalankan **LibreTranslate** di
komputer sendiri — MaxSubtitle akan otomatis memakainya duluan begitu
terdeteksi menyala, dan tetap fallback ke Google/MyMemory kalau servernya
mati, jadi aman diaktifkan kapan saja.

**1. Install & jalankan servernya (pilih salah satu):**

Lewat pip (paling ringan, langsung pakai Python yang sudah ada):
```
pip install libretranslate
libretranslate
```
Server akan menyala di `http://localhost:5000/`. Biarkan jendela
terminal ini tetap terbuka selama memakai MaxSubtitle.

Atau lewat Docker (kalau sudah terbiasa Docker):
```
docker run -ti --rm -p 5000:5000 libretranslate/libretranslate
```

**2. Install paket bahasa yang dibutuhkan.** Saat pertama kali dibuka,
buka `http://localhost:5000` di browser, lalu unduh paket bahasa sumber
& target yang dipakai (mis. Jerman & Indonesia). Kalau pasangan bahasa
langsungnya tidak tersedia, LibreTranslate otomatis menyambung lewat
Inggris asalkan kedua paket itu (sumber→Inggris dan Inggris→target)
sudah terpasang.

**3. Aktifkan di MaxSubtitle:** Pengaturan → nyalakan sakelar
**"Pakai server LibreTranslate lokal"** → pastikan URL sesuai (default
`http://localhost:5000/` sudah benar kalau memakai cara di atas) → Simpan.

Catatan bahasa: LibreTranslate (lewat paket yang dipakai MaxSubtitle)
mendukung 15 dari 18 bahasa target yang ada di Pengaturan. **Thai, Melayu,
dan Belanda belum didukung** — kalau salah satu dipilih sbg bahasa target,
MaxSubtitle otomatis lewati LibreTranslate dan langsung pakai Google/
MyMemory utk bahasa itu saja, tidak perlu diubah manual.
...
**Kolom Terjemahan isinya "Error 500 (Server Error)..." di semua baris**
Ini berarti Google Translate gratis sedang membatasi/menolak permintaan
(rate limit), dan versi lama MaxSubtitle salah mengira halaman error itu
sebagai hasil terjemahan yang valid. Sejak versi ini, tiap hasil
terjemahan divalidasi dulu — kalau ternyata halaman error atau pesan
kuota yang ketlisut, itu dianggap gagal (bukan diterima apa adanya),
lalu aplikasi otomatis mencoba lagi dan pindah ke mesin cadangan
(MyMemory) kalau Google terus gagal. Kalau ada baris yang tetap gagal
di semua mesin, teks ASLI baris itu dipertahankan (bukan pesan error)
dan aplikasi akan memberi tahu berapa baris yang terdampak lewat pesan
status setelah proses selesai — baris itu bisa dicoba diterjemahkan
ulang atau diedit manual.

**Terjemahan gagal / error rate limit**
Google Translate gratis kadang membatasi permintaan beruntun, apalagi
kalau video/subtitle-nya panjang (banyak baris dikirim berturut-turut).
Aplikasi sudah otomatis retry + jeda antar baris + pindah ke mesin
cadangan kalau perlu (lihat poin di atas), tapi kalau tetap banyak yang
gagal, tunggu beberapa menit lalu coba lagi, atau proses dalam batch
yang lebih kecil. Kalau butuh volume besar rutin tanpa batas sama sekali,
lihat bagian [Terjemahan Tanpa Limit (LibreTranslate Lokal)](#terjemahan-tanpa-limit-libretranslate-lokal).

**Video dengan beberapa bahasa berbeda dalam satu file**
Tiap baris subtitle diterjemahkan sendiri-sendiri dengan deteksi bahasa
otomatis ("auto"), jadi baris yang bahasanya berbeda-beda dalam satu
video tetap diterjemahkan dengan benar masing-masing — selama proses
terjemahannya sendiri berhasil (lihat 2 poin di atas). Catatan
terpisah: tahap TRANSKRIPSI (ucapan jadi teks) memakai Whisper, yang
mendeteksi SATU bahasa dominan untuk keseluruhan audio, bukan per
segmen — jadi kalau audio-nya sendiri (bukan cuma terjemahannya)
benar-benar berganti-ganti bahasa lisan sepanjang video, sebagian
segmen berbahasa lain mungkin kurang akurat ditranskrip. Ini batasan
Whisper, bukan sesuatu yang diperbaiki lewat pengaturan terjemahan.
